'use client'

// ─────────────────────────────────────────────────────────────────────────────
// Colabora con la ruta: Bizum para la gasolina.
//
// Bizum se hace desde la app del banco, así que aquí no hay pasarela. El número
// es el protagonista —grande y de una pieza— con un botón para copiarlo; el
// concepto va debajo, solo informativo. Se pide nombre y apellidos para poder
// llevar el registro de quién ha colaborado.
// ─────────────────────────────────────────────────────────────────────────────

import { useRef, useState } from 'react'
import { BIZUM, sinPrefijo } from '@/lib/reto50/constants'

const IconoCopiar = ({ className = 'w-3.5 h-3.5' }: { className?: string }) => (
  <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
  </svg>
)

const IconoOk = ({ className = 'w-3.5 h-3.5' }: { className?: string }) => (
  <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
  </svg>
)

export default function BloqueBizum({ config }: { config: Record<string, string> }) {
  const telefono = sinPrefijo(config.bizum_telefono || BIZUM.telefono)
  const concepto = config.bizum_concepto || BIZUM.concepto
  const titulo = config.bizum_titulo || 'Colabora con la ruta'
  const texto = config.bizum_texto || 'Ayuda a llenar el depósito para que el reto llegue a las 50 provincias.'

  const [estado, setEstado] = useState<'listo' | 'copiado' | 'manual'>('listo')
  const numeroRef = useRef<HTMLSpanElement>(null)

  async function copiar() {
    try {
      // Sin espacios: es como lo pide la app del banco.
      await navigator.clipboard.writeText(telefono.replace(/\s+/g, ''))
      setEstado('copiado')
    } catch {
      // Sin permiso de portapapeles: se deja seleccionado para copiarlo a mano.
      const nodo = numeroRef.current
      if (nodo) {
        const rango = document.createRange()
        rango.selectNodeContents(nodo)
        const sel = window.getSelection()
        sel?.removeAllRanges()
        sel?.addRange(rango)
      }
      setEstado('manual')
    }
    setTimeout(() => setEstado('listo'), 2500)
  }

  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 h-full flex flex-col text-center">
      {/* Título arriba, igual que el bloque del QR */}
      <h3 className="text-white font-black text-sm">{titulo}</h3>
      <p className="text-white/50 text-xs mt-1 leading-relaxed">{texto}</p>

      {/* El número: lo importante del bloque */}
      <div className="mt-3 bg-white/10 border border-white/15 rounded-xl px-3 py-3">
        <div className="text-[10px] font-black uppercase tracking-widest text-white/40">Bizum al</div>
        <span
          ref={numeroRef}
          className="block text-white font-black text-xl sm:text-2xl tracking-tight tabular-nums whitespace-nowrap select-all mt-0.5"
        >
          {telefono}
        </span>

        <button
          type="button"
          onClick={copiar}
          aria-label="Copiar el número de Bizum"
          className={`mt-2 inline-flex items-center gap-1.5 text-[11px] font-bold rounded-lg px-3 py-1.5 transition-colors ${
            estado === 'copiado'
              ? 'bg-emerald-500/20 text-emerald-300'
              : estado === 'manual'
                ? 'bg-amber-500/20 text-amber-200'
                : 'bg-white/10 text-white/80 hover:bg-white/20 hover:text-white'
          }`}
        >
          {estado === 'copiado' ? (
            <><IconoOk className="w-3 h-3" /> Copiado</>
          ) : estado === 'manual' ? (
            'Cópialo tú'
          ) : (
            <><IconoCopiar className="w-3 h-3" /> Copiar número</>
          )}
        </button>
        <span aria-live="polite" className="sr-only">
          {estado === 'copiado' ? 'Número copiado' : estado === 'manual' ? 'Número seleccionado para copiar' : ''}
        </span>
      </div>

      {/* El concepto es solo informativo: no hace falta copiarlo */}
      <p className="text-white/60 text-xs mt-2.5 leading-relaxed">
        Concepto: <strong className="text-white font-bold">{concepto}</strong> y tu{' '}
        <strong className="text-white font-bold">nombre y apellidos</strong>.
      </p>
      <p className="text-white/40 text-[11px] leading-relaxed mt-1.5">
        Así sabemos quién ha colaborado y podemos darte las gracias en el ranking de la ruta.
      </p>

      <a
        href="#gasolina"
        className="mt-auto pt-3 inline-flex items-center justify-center gap-1.5 text-xs font-bold text-pm-red hover:text-white transition-colors"
      >
        Ver el objetivo de gasolina
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
      </a>
    </div>
  )
}
