...
export default function FamiliasClient({ familias, links, alumnos, migrado, puedeEditar }: Props) {
  ...
  // Quién es socio se ve por su nº de socio (se asigna en Admin → Socios).
  const nSocios = familias.filter(f => f.numero_socio).length
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4"><div className="text-2xl font-black text-pm-navy">{nSocios}</div><div className="text-xs text-gray-400">Con nº de socio</div></div>
      </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
      <div className="flex items-center gap-3 px-5 py-4 flex-wrap">
        ...
        {familia.numero_socio && (
          <span title="Socio del Club" className="text-xs font-bold bg-pm-navy/5 text-pm-navy border border-pm-navy/15 rounded-full px-2 py-0.5 whitespace-nowrap">⭐ {familia.numero_socio}</span>
        )}
        {puedeEditar ? (
      ...
      {abierta && (
        <div className="border-t border-gray-100 p-5 space-y-4">
          ...
          {/* El nº de socio se asigna en Admin → Socios; aquí solo se consulta. */}
          <div className="border-t border-gray-100 pt-4 text-xs text-gray-500">
            {familia.numero_socio
              ? <>Socio <strong className="text-pm-navy">⭐ {familia.numero_socio}</strong>. Entra al portal con su correo y este número.</>
              : <>Sin nº de socio: todavía no puede entrar al portal.</>}
            {' '}El número se asigna en <a href="/admin/socios" className="text-pm-red font-semibold hover:underline">Socios</a>.
          </div>
